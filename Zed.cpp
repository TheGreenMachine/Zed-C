...
void Zed::OperatorControl()
{
    ...
    ToggleHelper OrientationToggle;
    while (IsEnabled() && IsOperatorControl())
    {
        ...
        if (OrientationToggle(comps.driver.GetRawButton(1)))
        {
            orientation *= -1;
        }
        ...
        if (comps.shooter.GetRawButton(6))
        {
            ...
            comps.augerMotor.setState(Auger::AUGER_OVERRIDE_FORWARD);
        }
        else if (comps.shooter.GetRawButton(10))
        {
            //comps.augerMotor.setState(Auger::AUGER_LOADING_START);
            comps.augerMotor.setState(Auger::AUGER_OVERRIDE_REVERSE);

        }
        //TODO Remove this once we revert to limit switches, the idling is automatic then.
        else
        {
            comps.augerMotor.setState(Auger::AUGER_IDLE);
        }
...
void Zed::mechanismSet()
{
    ...
    //Drive
    comps.driveTrain.drive(speedX * orientation, speedY * orientation,
